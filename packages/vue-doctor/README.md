...
Built for AI agents, coding agents, and CI: a stable, color-free document on
stdout (`schema: "vue-doctor/diagnosis@1"`) with `score`, `project`, `summary`,
and a `diagnostics[]` array where each item carries `file`, `line`, `column`,
`severity`, `category`, `rule`, `message`, and a `fix` hint.
